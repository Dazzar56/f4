...
*   **Step 13b — a saved position that belongs to one file only.** `file_states.json` is keyed by the bare path, and a path is unique only within one file system: `/etc/hosts` on a remote host and `/etc/hosts` here are one entry, and two hosts with the same layout are one entry as well. Reopening a file must land where it was left; reopening a namesake somewhere else must not. The key has to carry which file system the path was on, and the existing file has to survive the change rather than being read as if it already did.
