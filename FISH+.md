...
*   **Step 11b — the remote terminal.** A window over the `exec` job, and then an interactive one: a shell on a pseudo terminal of its own on the far side, which needs a second channel rather than the request stream. far2l's built-in terminal is the reference for what the interactive half has to get right.
