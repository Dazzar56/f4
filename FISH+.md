...
*   **Step 13 — Also: users report that viewer is loading the whole file at start. Also, it is unclear is viewer and editor use remote search. This looks like critical regression.
*   **Step 13 — the editor.** Whether a remote line index helps the editor at all is the open question, and the answer looks like no: the editor loads a file into a piece table whose original buffer is the file itself, so by the time anything could be counted the bytes are already here. What would help is not counting but loading — an editor that opens a large remote file without fetching all of it first — and that is a different piece of work from step 7b. Worth measuring before building.
