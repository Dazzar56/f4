...
func TestPanelsFrame_KeyBar_ProcessMouse(t *testing.T) {
	vtui.FrameManager.Init(vtui.NewSilentScreenBuf())
	SetDefaultF4Palette()
	...
	// Simulate left click on the bottom row (y = 24)
	ev := &vtinput.InputEvent{
		Type:        vtinput.MouseEventType,
		KeyDown:     true,
		MouseX:      10, // within F1 or F2 bounds
		MouseY:      24,
		ButtonState: vtinput.FromLeft1stButtonPressed,
	}
	...
	handled := pf.ProcessMouse(ev)
	if !handled {
		t.Error("Expected mouse click on KeyBar row to be handled by PanelsFrame")
	}
}
func TestPanelsFrame_KeyBar_ActionDispatch(t *testing.T) {
	vtui.FrameManager.Init(vtui.NewSilentScreenBuf())
	SetDefaultF4Palette()
	...
	// Mock file panel so Copy has something to act on
	fsp := pf.panels[0].(*FileSystemPanel)
	...
	// Simulate left click on F5 Copy (the KeyBar splits 12 buttons across 80 cols,
	// so each button is roughly 6-7 cols wide. F5 is around column 30).
	ev := &vtinput.InputEvent{
		Type:        vtinput.MouseEventType,
		KeyDown:     true,
		MouseX:      30, // within F5 bounds
		MouseY:      24,
		ButtonState: vtinput.FromLeft1stButtonPressed,
	}
	...
	handled := pf.ProcessMouse(ev)
	if !handled {
		t.Error("Expected F5 KeyBar click to be handled")
	}

	// Verify that the Copy confirmation dialog is opened
	top := vtui.FrameManager.GetTopFrame()
	if top == nil || !strings.Contains(top.GetTitle(), "Copy") {
		t.Errorf("Expected Copy dialog on top, got %v", top)
	}
