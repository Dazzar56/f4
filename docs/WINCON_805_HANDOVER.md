...
уничтожения ввод вернулся. То есть на этой сборке жалоба «мелькает, клин,
17% CPU» — это F7/F8 (сцепка очередей ввода), а не затирание. F6 остаётся
...
**F23. Layered-окно шага 3 проверено на машине и работает.** Тот же прогон:
...
не заморозил. Спецификация шага 3 подтверждена на реальной сборке, а не только
выведена. Один ответ требует перепроверки: тестер сообщил, что окно «поехало
вместе с консолью», хотя трекера в пробнике нет и оно двигаться не должно.

**F24. Sixel в WT привязан к текстовому буферу: прокрутка его уносит.**
Полевой прогон под WT 1.24.2607.10001: картинка пережила текст рядом, `ESC[K`
по своим строкам, альтернативный экран и двадцать перерисовок подряд без
единого сбоя. Единственное, что её убивает, — **прокрутка**: изображение
уезжает вверх вместе со строками, обрезается верхней границей окна и в конце
концов исчезает. Следствие для f4: после любой прокрутки картинку надо
перерисовывать; ресайз ведёт себя так же, потому что тоже прокручивает.
Ничего похожего на «то показывается, то нет» при этом не наблюдалось —
значит исходная жалоба объясняется случаем (a) (нет `WT_SESSION` → протокол
не выбран), а не потерей уже нарисованного.
...
- **Q9.** Почему layered-окно в прогоне F23 сдвинулось вместе с окном консоли,
  хотя трекера в пробнике нет. Перепроверить одним прогоном.
...
Статус: механизм проверен на 22000 (F23). Обоснование — F7, а не F6: на этой
