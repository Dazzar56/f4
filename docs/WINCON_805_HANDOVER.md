...
- `GetConsoleProcessList` видит только сам процесс; conhost — ребёнок процесса,
  создавшего псевдоконсоль (P4 из `TERMINAL_LEDGER.md`), а не ребёнок шелла.
...
Статус: подход подтверждён замерами (F13, F14) — WT отвечает `4`, conhost 22000
...
Для полного диагностического прогона используйте `tools/conptyprobe` (инструкция
в его `README.md`). Версия 6 сама запускает forced conhost, явный Windows
