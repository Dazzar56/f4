...
2.  **No Active Reflow, for now (and why that is not a law of nature):** `f4` does not currently reflow (re-wrap) the live 2D grid on resize. It uses **Horizontal Preservation** for the live grid (hiding off-screen characters without deleting them) and relies on the `PieceTable`'s `WrapEngine` to reflow when the user views the scrollback history (e.g. via `Ctrl+O`).

    This section used to claim that reflowing the live grid is *impossible* because the shell tracks its own cursor and would desync. That claim is wrong, and it was talking anyone who tried out of a working design. `far2l` reflows exactly this live grid and it works, because of two decisions worth copying (`WinPort/src/ConsoleBuffer.cpp`, `SetSizeRecomposing`):
