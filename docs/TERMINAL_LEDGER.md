...
| O11 | Scrollback does not come back in the field (§6.7, §6.8, §6.10). | **Diagnosed: the re-wrap is lossy.** `REFLOW_WRAP:` shows the reconstructed logical-line count falling on nearly every pass -- 1858, 1815, 1520, 1025, 817, 744, ... 10, 1 -- until `GridHistory` collapses to zero. Not overwriting (the absorber ruled that out), not collection (`unwrapLocked` gathers the history correctly). Windows-only because a ConPTY drag runs the pass per pixel step: 183 passes in one run. Next: a round-trip losslessness test over a sequence of widths, before any fix. |
