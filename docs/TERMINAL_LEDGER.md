...
| O11 | Scrollback does not come back in the field, on either host (§6.7). | The log shows the run used `F4_WIN_REFLOW=probe`, where `ReflowOnResize` is false and no re-wrap happens by design: 488 resizes, zero reflows, 84 more rows extruded to history, zero pulled back. Repeat with `oracle` (the Windows default) before anything else. Established regardless of mode: the oracle fired twice in the whole session and never during a resize, so the history's boundaries come from the `ESC[K` hint in practice, not from the oracle. |
