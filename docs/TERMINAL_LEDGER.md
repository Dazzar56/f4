...
| O11 | Scrollback does not come back in the field (§6.7-§6.16). | Two causes, both ConPTY repaints reaching the display in the gaps between lossless re-wrap passes: height-only steps of a corner drag (§6.15, absorber gated on width) and same-size resize events that still called `ResizePseudoConsole`, which repaints even for an identical size and then without a size report (§6.16). Fixed: absorb on any resize; tell ConPTY nothing when nothing changed; absorb whatever does reach it; recognise size-less frames in the log. |
