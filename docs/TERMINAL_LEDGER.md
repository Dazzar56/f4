...
| O18 | Direction A is unproven: does it help anything in practice? | Measured once on 10.0.22000 (`CONPTY_RESEARCH.md` §8): PowerShell 5.1 produces nothing without a console, so it needs one; WSL was not installed; `pwsh` was not installed; `ssh.exe` starts over pipes but was not shown to carry a usable width. Needs one machine with WSL and PowerShell 7, plus `ssh -t <host> "stty size; tput cols"` run from f4 and from cmd. Incidental: `wsl.exe` writes UTF-16LE to a pipe. |
...
| O17 | ~~Can f4 hold `\Device\ConDrv\Server`, receive console API messages, and have conhost accept that handle?~~ | **Closed: D2 is not viable.** The endpoint is creatable unprivileged, the server role is obtainable and exclusive, and conhost accepts an endpoint f4 created -- but f4 cannot place a client on it: `\Connect` relative to a server handle returns `STATUS_INVALID_HANDLE`, because the kernel creates the client side on behalf of `AllocConsole`. Doing that ourselves means reimplementing part of `AllocConsole` on an undocumented ioctl, which is D's cost without D's guarantees. See `CONPTY_RESEARCH.md` §8. |
