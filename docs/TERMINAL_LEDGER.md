...
| P22 | The corrected Idea-C probe kept a ConPTY at 4000 columns on 10.0.22000.2538 and delivered 184- and 3968-character lines as one cursor-model row. Height-only resizes 4000x29 → 4000x30 → 4000x31 → 4000x30, plus a post-resize line, all passed with `whole=true`, `split=false`, `rows=1`. | P | wide-ConPTY premise measured successfully on this build; f4 integration and newer Windows builds remain open |
...
| O9 | The XTWINOPS report (P14) as the repaint delimiter. **Now a prerequisite, not a nicety (§6.8).** Since 6.16 the absorber covers every resize without it; it remains the way to tie a late `STALE` frame to its resize. | Today the scratch-view routing keys on `ESC[?25l … ESC[?25h`. `ESC[8;h;wt` says the size as well, so a frame can be matched to the resize that caused it, and a frame that is not ours can be told apart. |
| O11 | ~~Scrollback does not come back in the field~~ (§6.7-§6.17). | **Closed, confirmed in the field.** Two causes, both ConPTY repaints landing on a display f4 had already superseded: height-only steps of a corner drag, and same-size resize events that still called `ResizePseudoConsole`, which repaints even for an identical size and then without a size report. The re-wrap itself was never at fault. Also fixed on the way: the history is bounded in logical lines, not rows. |
| O13 | ~~The repaint absorber may swallow ordinary output.~~ | **Closed.** It did, in the field: a resize during `dir` blanked the middle of the listing, because the absorber keyed on timing and on the cursor hide, which every ConPTY batch carries (§6.18). A repaint is now recognised by shape, only when ConPTY owes one, and never on the alternate screen (§6.19), so output cannot match. Reproduced on the mocks before the fix; `TestResizeDuringCommandDoesNotEatOutput` and `TestLongScrollingDirWithResizesLosesNothing` hold it. |
| O14 | A `cls` issued in the same breath as a resize can have its repaint counted as the one ConPTY owed. | Recorded, not fixed: one visible, recoverable frame, never output. Fixing it needs O9 -- tying a frame to the resize that caused it by its size report. |
| O15 | Resize f4's grid when ConPTY's frame for that size arrives, not when the user drags. | Would make the grid always match the size the stream is laid out for, closing the residual window of §6.22 (a command pausing >200 ms in its first second) and the STALE-frame class for good. Needs the size report (O9) on 22000 and the pending size on 19045. Not started. |
| O16 | Does `src/host` from `microsoft/terminal` build standalone, and does the result serve a console when launched with `--server <handle>`? | The question that decides direction D (`CONPTY_RESEARCH.md` §8): the console's wrap flag (`ROW::wrapForced`) is exported by no public API, so only the process owning the buffer can read it. One evening to answer; the answer changes what kind of problem this is. |
| O17 | Can f4 hold `\Device\ConDrv\Server` itself, receive console API messages, and have the system conhost accept that handle via `--server`? | The question behind direction D2 (`CONPTY_RESEARCH.md` §8): proxy the console server rather than reimplement it, and read the application's intent before anything wraps it. **Partly answered on 10.0.22000:** the endpoint is creatable unprivileged, and the system conhost accepted a handle f4 created (`--server`). Whether f4 can read the messages itself is still open -- the first probe used the wrong device code (0x8000 for `FILE_DEVICE_CONSOLE` instead of 0x50) and never called `SET_SERVER_INFORMATION`; both fixed. Second run: `SET_SERVER_INFORMATION` accepted (the server role is obtainable), and conhost refused the same handle it had accepted before the probe claimed that role -- so an endpoint has exactly one server, first claimant wins, and D2 must be a real proxy (two endpoints, forwarding), never passive listening. Question 2 still open: the probe attached its client with a plain `CreateProcess`, which hands the child the parent's console; it now opens `\Reference`, `\Input` and `\Output` under the server handle and passes them as the child's standard handles. Stability so far: the ConDrv IOCTL set is unchanged since Windows 8.1 (2013), `conhost --server` unchanged since 1809 (2018), headers published in `microsoft/terminal` under `dep/Console`. |
