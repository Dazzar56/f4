...
| O11 | Scrollback does not come back in the field (§6.7-§6.14). | Open. Established: the re-wrap is lossless in isolation (full resize-shape matrix), the row cap was a real cause and is fixed, the repaint is not overwriting, the oracle stamps but almost never runs. Retracted: the "all counters zero" conclusion of §6.13 -- two counters could not observe their subject and are fixed. Next, in order: a baseline run with `off` and `hint` (this session changed the default, added the absorber and changed the history bound); then the corrected counters, including what the absorber swallows and blanks trimmed from wrapped rows. |
