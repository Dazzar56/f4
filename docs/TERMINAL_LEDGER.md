...
| O13 | The repaint absorber may swallow ordinary output. | Its window is up to 250ms and a drag opens it on every width change (174 times in one run), so windows overlap. The test behind "ordinary output cannot be swallowed" only checked that the diversion closed. It now logs bytes absorbed and whether they formed a delimited frame; an undelimited absorb is a confirmed bug. |
