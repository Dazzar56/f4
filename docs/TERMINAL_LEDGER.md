...
| O11 | Scrollback does not come back in the field (§6.7-§6.13). | The row cap was one cause and is fixed (evictions 349 -> 5 per session, §6.12), but the decay continues: input cells 107781 -> 74986 and logical lines 2025 -> 1457 over 472 passes, with every counter at zero. The loss is inside the unwrap -> re-wrap round trip itself. Two candidates: wrap flags from the `ESC[K` hint merging lines that really ended (P13, re-guessed hundreds of times during a drag), and `significantWidthLocked` trimming real trailing spaces (A5). Next: a pure round-trip identity test, no field run needed. |
