...
| O17 | Can f4 hold `\Device\ConDrv\Server` itself, receive console API messages, and have the system conhost accept that handle via `--server`? | The question behind direction D2 (`CONPTY_RESEARCH.md` §8): proxy the console server rather than reimplement it, and read the application's intent before anything wraps it. `tools/condrvprobe` takes the measurement; it needs reports from several Windows builds, because the protocol is undocumented and only agreement across builds is evidence. Stability so far: the ConDrv IOCTL set is unchanged since Windows 8.1 (2013), `conhost --server` unchanged since 1809 (2018), headers published in `microsoft/terminal` under `dep/Console`. |
