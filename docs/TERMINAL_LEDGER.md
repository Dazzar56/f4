...
| O11 | Scrollback does not come back in the field (§6.7, §6.8). | Diagnosed. Not the oracle (25 boundaries stamped in one pass, none stale, no disagreement) and not the history (4728 rows extruded intact). ConPTY sends a full-viewport repaint on every resize step -- 197 across 444 events -- and it lands on the display after f4 re-wraps, overwriting the recovered rows; since ConPTY keeps no scrollback (P16), a shrink-and-expand repaints blanks over the history. Fix: accept the repaint as structure and stitch at the seam (§3.3 last paragraph), which needs O9 first. |
