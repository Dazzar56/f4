...
| O11 | Scrollback still does not reflow in the field, on either host (§6.7). Rows that scrolled off are not restored on a widen; a correct-looking history flashes during the drag and is replaced; after a large shrink and re-expand the freed area stays black. | Undiagnosed. Next step is a `F4_WIN_REFLOW=probe --debug` log of that exact resize sequence, read for three things: whether the missing rows were stamped, whether f4's repaint overwrites ConPTY's frame and in which order, and the `GridHistory` row count across the shrink. No code changes before that log. |
