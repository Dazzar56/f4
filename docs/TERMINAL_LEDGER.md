...
| O11 | Scrollback does not come back in the field (§6.7-§6.15). | **Found and fixed.** Characters were preserved inside every re-wrap pass and lost only between them; the cause is ConPTY's repaint landing on height-only resize steps, where the absorber was gated on width. A corner drag interleaves width and height steps, so every previous hypothesis about the re-wrap was tested on a log where the re-wrap was innocent. The absorber now covers any size change. Two `STALE` frames remain unexplained and motivate O9. |
